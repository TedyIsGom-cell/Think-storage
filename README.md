...
- 댓글: 누구나 이름 + 비밀번호 + 내용으로 작성 가능, **삭제는 관리자만** 가능
  (관리자로 로그인한 상태에서 글 페이지에 들어가면 각 댓글에 "삭제" 버튼이 보입니다)
...
> ⚠️ 데이터베이스 구조(prisma/schema.prisma)가 바뀌었으므로, 이미 한 번 배포한 상태라면
> 로컬에서 `npx prisma db push` 를 다시 한 번 실행해서 `viewCount`, `likeCount`, `Comment` 테이블을
> 추가해줘야 합니다. (위 "3. 데이터베이스 연결하기" 4번 항목과 동일한 명령어입니다.)
